...
const BillingSummary = ({ subt, disc, onDisc, gst, total }) => {
  ...
  return (
    <div className="p-3 border-top border-bottom bg-light bg-opacity-10">
      <div className="d-flex justify-content-between mb-2 small"><span className="text-muted small">Cart Subtotal</span><span className="fw-bold">₹{subt.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
      ...
      <div className="d-flex justify-content-between mb-2 small">
        ...
        <span className="fw-bold">₹{gst.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
      </div>
      <div className="d-flex justify-content-between align-items-center border-top pt-2"><span className="fw-bold text-dark h6 mb-0">Grand Total</span>
        <span className="fw-bold text-primary h4 mb-0">₹{total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
    </div>
  );
};
